...
describe("gouraud vs phong on a curved surface", () => {
  ...
  it("phong carries per-vertex base color through, not material.color", () => {
    const vertex: ShadingVertex = { viewPos: vec3(0, 0, -5), viewNormal: vec3(0, 0, 1) };
    const shade = makePhongShadeFn(light, material);
    const red = shade(phongVertexAttrs(vertex, vec3(1, 0, 0)), 0, 0);
    const blue = shade(phongVertexAttrs(vertex, vec3(0, 0, 1)), 0, 0);
