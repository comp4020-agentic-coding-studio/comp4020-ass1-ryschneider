...
/** Unit cube (extent -0.5..0.5 per axis), 6 faces x 2 triangles, flat per-face normals. */
...
/** A flat 2x2 (extent -1..1) horizontal plane facing +Y, 2 triangles. */
export function planeMesh(): Mesh {
  return buildMesh([
    { verts: [vec3(-1, 0, 1), vec3(1, 0, 1), vec3(1, 0, -1), vec3(-1, 0, -1)], normal: vec3(0, 1, 0) },
  ]);
}
